...
LAMBBDA = 100
 ...
 def generator_loss(disc_generated_output, gen_output, target):#생성자의 손실함수 정의?
  ...
  이 부분이 제일 문제이다 이전에 cgan의 손실함수에 대해 소개한적이 있는데 그와 형태가 내가 봤을때에는 매우 상이하여 어쨰서 이런 코드로 
  생성자의 loss를 구현했는지 모르곘다.. 일단은 tensorflow에서 하란대로 하긴했는데 이후에 다른예제를 찾아보면서 알아봐야 겠다.  
  
  모르는 점을 정리해보자면 
  1. loss_object란 함수자체를 모르겠다 - 하지만 sigmoid loss의 개념인듯 싶다.  
  2. 그 인자에 disc_generated_output shape 의 배열과 disc_generated_output가 동시에 들어가는지
  ...
  down1 = downsample(64, 4, False)(x) # (bs, 128, 128, 64) 애내는 왜또 이렇게 들어가냐?
  ...
  zero_pad2 = tf.keras.layers.ZeroPadding2D()(leaky_relu) # (bs, 33, 33, 512) #마찬가지
  ...
  ```
  여전히 이해 않가는것 투성이다.. gan에 대한 이해과정이 부족해서 그런가 명쾌하지가 않다. input
