...
def train_step(input_image, target, epoch):
 ...
 지금까지 작성해온 C-GAN의 핵심이라고 볼 수 있는 부분이다 생성자(generator)모델과 구분자(discriminaotr) 모델의 학습을 위에서 코드로 구현하였던 손실함수에 인수로 전달하여 적대적 학습을 하고있다. 이때 tape에 학습값이 저장되므로 계속해서 이어나갈 수 있다.
